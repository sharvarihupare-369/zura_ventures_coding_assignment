  ...
  return (
    <div>
      <div className="flex justify-between m-3">
        ...
        <div className="">
          <button onClick={()=>setIsModalOpen(true)} className="flex justify-center mr-20 py-2 bg-[#211935]  text-white text-[20px]    rounded-lg flex gap-2 items-center">
            <img
   ...
   {/*projects map here */}
      <div className="w-[75%]  m-auto grid grid-cols-3 gap-x-20">
         {allProjects && allProjects?.map((project)=>{
          ...
          const initialOfName = project?.projectName?.split(" ").map((word)=>word[0]).join("").toUpperCase()
        ...
        >
          <div className="flex gap-4 ">
            <div className="bg-[#7E22CE] inline-block rounded-md">
              <p className="text-white text-[40px] font-bold py-2 px-4">{initialOfName}</p>
            </div>
            <div className="mt-2">
              <p className="text-[#7E22CE]  font-bold">{project?.projectName}</p>
